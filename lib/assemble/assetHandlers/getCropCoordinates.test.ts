...
describe('.frames parser', () => {
    test('Parsing correct frames', () =>
        expect(parseFrameGrid(testFramesComplex).status).toEqual('success'));

    describe('Parsing with warnings', () => {
        test('Incorrect width', () => {
            const modifiedResult = parseFrameGrid(
                (() => {
    ...
    test('.frames with number keys', () =>
        expect(parseFrameGrid(testFramesKeyNumber).status).toEqual('success'));
});

describe('getCropCoordinates', () => {
    test('Simple object', () =>
        expect(getCropCoordinates('full', testFramesSimple)).toEqual({
            left: 16,
            top: 0,
            width: 16,
            height: 16,
    ...
    describe('Simple object with coerceable number key', () => {
        test('Directly', () =>
            expect(getCropCoordinates('1', testFramesKeyNumber)).toEqual({
                left: 16,
                top: 0,
                width: 16,
                height: 16,
        ...
        test('By alias', () =>
            expect(getCropCoordinates('default', testFramesKeyNumber)).toEqual({
                left: 0,
                top: 0,
                width: 16,
                height: 16,
    ...
    describe('Simple object with frameList', () => {
        test('Directly', () =>
            expect(getCropCoordinates('run', testFramesList)).toEqual({
                left: 43,
                top: 86,
                width: 43,
                height: 43,
        ...
        test('By alias', () =>
            expect(getCropCoordinates('idle.4', testFramesList)).toEqual({
                left: 0,
                top: 43,
                width: 43,
                height: 43,
    ...
    describe('Complex object', () => {
        test('Directly', () =>
            expect(
                getCropCoordinates('firewindup.4', testFramesComplex),
            ).toEqual({
                left: 192,
                top: 128,
                width: 64,
        ...
        test('By alias', () =>
            expect(
                getCropCoordinates('fireprojectile.4', testFramesComplex),
