...
describe('getFrameFromObject', () => {
    ...
    test('Actual path', () =>
        expect(
            getFramesFromObject(testFrameObj, 'items/active', 'someitem'),
        ).toEqual('priority1'));
    test('Actual path, debug', () =>
        expect(
            getFramesFromObject(testFrameObj, 'items/active', 'someitem', true),
        ).toEqual('items/active/someitem.frames'));
    test('Actual path, fallback to default', () =>
        expect(
            getFramesFromObject(testFrameObj, 'items/active', 'someotheritem'),
        ).toEqual('priority2'));
